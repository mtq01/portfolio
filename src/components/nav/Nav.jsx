...
function Nav() {
  const [lightToggle, setLightToggle] = useState("dark");
  const current = palette[lightToggle];

  return (
    <nav className="site-nav">
      {/* Logo */}
      <img src="/src/assets/react.svg" alt="temp logo" class="site-logo" />
      ...
      <div
        ...
        onClick={() =>
          setLightToggle(lightToggle == "darkMode" ? "lightMode" : "darkMode")
        }
      >
        <div className="nav-link">
          {/* swap icon & text based on state */}
          ...
          <span className="nav-icon-title">
            {lightToggle === "darkMode" ? "Dark" : "Light"}
          </span>
        </div>
      </div>
    </nav>
